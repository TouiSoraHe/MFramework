namespace MFramework.Common
{
    ...
    public static class LogEditor
    {
        ...
        {
            new LogEditorConfig(Log.GetThisClassRelativePath(), typeof(Log))
        };
            ...
            for (int i = _logEditorConfig.Length - 1; i >= 0; --i)
            {
                ...
                if (instanceID == configTmp.instanceID)
                {
                    ...
                    if (!string.IsNullOrEmpty(statckTrack))
                    {
                        ...
                        UnityEngine.Object target = AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(fileName);
                        AssetDatabase.OpenAsset(target, fileLine);
                        return true;
                    }
                    break;
                }
