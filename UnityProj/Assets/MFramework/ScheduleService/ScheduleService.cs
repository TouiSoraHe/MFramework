...
namespace MFramework.ScheduleService
{
    /// <summary>
    /// 调度服务
    /// </summary>
    public class ScheduleService : SingletonMB<ScheduleService>
    {
        private struct Data
        {
            // 调度ID
        ...
        private readonly List<IScheduleHandler> _updates = new List<IScheduleHandler>(50);
        private readonly List<Data> _timers = new List<Data>(20);
        private readonly List<Data> _frames = new List<Data>(20);
        ...
        protected void Update()
        {
            ...
            for (int i = 0; i < count && i < _updates.Count; i++)
            {
                ...
                if (handler != null)
                {
                    ...
                    {
                        Log.LogE("ScheduleService.Update : update exception {0} at handler {1}", e, handler.GetType().Name);
                        throw;
                    }
                }
                else
            ...
            for (int i = 0; i < count && i < _timers.Count; i++)
            {
                ...
                {
#if SSK_PROFILE
                    ProfilerSample.BeginSample(handler);
#endif
                ...
                {
                    Log.LogE("ScheduleService.Update : timer exception {0} at handler {1}", e, handler.GetType().Name);
                    throw;
                }
            }
            ...
            for (int i = 0; i < count && i < _frames.Count; i++)
            {
                ...
                {
                    Log.LogE("ScheduleService.Update : frame exception {0} at handler {1}", e, handler.GetType().Name);
                    throw;
                }
            }
        ...
        private void Remove(List<Data> datas, uint id)
        {
        ...
        private void Remove(List<Data> datas, IScheduleHandler handler)
        {
            for (int i = 0; i < datas.Count; i++)
            {
